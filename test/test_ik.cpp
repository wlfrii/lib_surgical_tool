...
bool IKC3(const mmath::Pose &pose, float L1_max, float Lr,
                        float L2, float Lg, SurgicalToolConfig &config);
...
int main()
{
    ...
    float Lg = 5.71;
    Eigen::Matrix4f T;
    T << 0.958300131788949, -0.252909872726839, 0.133031776995487, -5.396805608642865,
            0.228442897054108, 0.957685179182800, 0.175079811397928, -28.614950740329022,
            -0.171681974006618, -0.137388841799114, 0.975525297442505, 63.043203955522713,
            0, 0, 0, 1;

    IKC3(mmath::Pose(T), L1, Lr, L2, Lg, config);

    printf("---------------------------\n");

    T << 0.210556828377472, -0.161892079703181, 0.964083386721812, 23.687447266090576,
         0.670329975244716, 0.741741543294745, -0.021845073566982, -39.699102970516300,
        -0.711564154740834, 0.650853622161011, 0.264699860611702, 55.854892705416283,
                         0,                   0,                   0,   1.0;
    IKC3(mmath::Pose(T), L1, Lr, L2, Lg, config);


//    std::complex<float> res = sqrt(std::complex<float>(5, 0));
//    printf("test: %f + %fi\n", res.real(), res.imag());
 ...
 *         false - out of workspace
 */
bool IKC3(const mmath::Pose &pose, float L1_max, float Lr, float L2, float Lg,
          SurgicalToolConfig &config)
{
    bool flag = true;

    float theta1_max = M_PI / 2.0;
    float theta2_max = 2.f*M_PI / 3.0;
    ...
    printf("l1:%f, costheta1:%f\n", l1, costheta1);

    config.theta1 = theta1;
    config.L_insert = L1;
    printf("config.L_insert:%f, config.theta1:%f\n", config.L_insert, config.theta1);
...
//        float tol = 1e-1; // unit circle slack
        flag = false; // r1 lim

        // control the density of boundary points, at least 400
//        float dt2 = (theta2_max - t2)/600;
//        std::vector<float> t2s;
//        mmath::linspace(t2, dt2, theta2_max, t2s);
